...
class Monkey {
    ...
    protected _items: bigint[];
    protected operation: (old: bigint) => bigint
    protected test: (worryLvl: bigint) => number
    ...
    constructor(monkeyInput: string, protected monkeyLevelDivision: number | null) {
        ...
        this._items = lines[1].substring(18).split(",").map(i => BigInt(i));
        this.operation = (old) => {
            const oldOrValue = (v: string) => v === "old" ? old : BigInt(v);
        ...
        const divisible = BigInt(lines[3].substring(21));
        const trueCondition = parseInt(lines[4].substring(29));
        const falseCondition = parseInt(lines[5].substring(30));
        this.test = (worryLvl) => worryLvl % divisible === BigInt(0) ? trueCondition : falseCondition;
    }
